/**
 * DashboardLayout — Authenticated dashboard shell with header and sidebar.
 * Desktop: full sidebar + header + content
 * Tablet: collapsible sidebar + header + content
 * Mobile: header + content + bottom nav
 *
 * Reference: DeutschFlow Design Bible 03_LAYOUTS/002_Dashboard_Shell.md
 */
...
interface DashboardLayoutProps {
  children: ReactNode;
  /** Hides the sidebar — used for full-width pages like chat */
  hideSidebar?: boolean;
}

export function DashboardLayout({ children, hideSidebar }: DashboardLayoutProps) {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen" style={{ background: "var(--color-background-primary)" }}>
      ...
      >
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex items-baseline gap-0.5">
            ...
            <span className="text-xl font-bold tracking-[1px]" style={{ background: "var(--color-accent-gradient)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Flow</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {/* Search */}
          <button onClick={() => setCommandOpen(true)} title="Search (Ctrl+K)" aria-label="Open search"
            className="hidden sm:flex items-center gap-2 pl-3 pr-2 py-1.5 rounded-full text-xs transition-colors hover:bg-white/5"
            style={{ color: "var(--color-text-muted)", border: "1px solid var(--color-border-subtle)", background: "rgba(255,255,255,0.02)" }}>
          ...
          <button title="Notifications" aria-label="View notifications" onClick={() => router.push("/notifications")}
          ...
          <div className="relative" ref={menuRef}>
            ...
            {userMenuOpen && (
              <div className="absolute right-0 mt-1.5 w-56 rounded-xl py-1 shadow-lg z-50" style={{ background: "var(--color-surface-1)", border: "1px solid var(--color-border-subtle)", boxShadow: "var(--elevation-3)" }}>
                <button onClick={() => { setUserMenuOpen(false); router.push("/settings"); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-white/5" style={{ color: "var(--color-text-secondary)" }}>
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <button onClick={() => { setUserMenuOpen(false); logout(); router.push("/"); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-white/5" style={{ color: "var(--color-text-secondary)" }}>
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="grid grid-cols-5 gap-1.5 px-3 py-2.5">
                  {THEME_LIST.map((t) => (
                    <button key={t.key} onClick={() => setTheme(t.key)} title={t.label}
                      ...
                      style={{ background: t.color, boxShadow: theme === t.key ? `0 0 10px ${t.color}99` : "none", outline: theme === t.key ? `2px solid ${t.color}99` : "none", outlineOffset: "1px" }}>
                      {theme === t.key && <span style={{ fontSize: "8px", color: t.label === "Onyx" ? "#000" : "#fff" }}>✓</span>}
                    </button>
                  ))}
                </div>
                <div className="px-3 pb-2 text-center" style={{ color: "var(--color-text-muted)", fontSize: "10px" }}>{THEME_LIST.find(t => t.key === theme)?.label}</div>
              </div>
            )}
          </div>
        </div>
