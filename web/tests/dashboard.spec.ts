 ...
 *
 * Some tests check desktop-only layout sections and are skipped on
 * mobile projects via testInfo.project.name.
 */
...
test.describe("Dashboard — Authenticated", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.getByText(/guten/i).first()).toBeVisible({ timeout: 15000 });
  });

  test("renders greeting with user name", async ({ page }, testInfo) => {
  ...
  test("displays current level card with progress bar", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes("Mobile"), "Desktop-only layout");
  ...
  test("shows Progress section with ring", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes("Mobile"), "Desktop-only layout");
    await expect(page.getByText("Your Progress").first()).toBeVisible();
    await expect(page.getByText("35% complete").first()).toBeVisible();
  });

  test("shows KPI cards with correct values", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes("Mobile"), "Desktop-only layout");
  ...
  test("shows Review & Practice section", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes("Mobile"), "Desktop-only layout");
  ...
  test("shows Recent Activity section", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes("Mobile"), "Desktop-only layout");
    await expect(page.getByText("Recent Activity").first()).toBeVisible();
  });

  test("shows Tip of the Day", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes("Mobile"), "Desktop-only layout");
    await expect(page.getByText("Tip of the Day").first()).toBeVisible();
  });

  test("View Roadmap button navigates to /curriculum", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes("Mobile"), "Desktop-only layout");
...
test.describe("Dashboard — Empty State (new user)", () => {
  ...
  test("shows 'Ready to begin' when no progress", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes("Mobile"), "Desktop-only layout");
    await expect(page.getByText("Ready to begin").first()).toBeVisible();
  });

  test("shows 'Start today!' when streak is 0", async ({ page }) => {
    await expect(page.getByText("Start today!").first()).toBeVisible();
  });

  test("shows 'Your journey begins' for empty activity", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes("Mobile"), "Desktop-only layout");
    await expect(page.getByText("Your journey begins").first()).toBeVisible();
  });

  test("shows 0% quiz accuracy for new user", async ({ page }, testInfo) => {
    test.skip(testInfo.project.name.includes("Mobile"), "Desktop-only layout");
